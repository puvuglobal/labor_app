...
CREATE POLICY IF NOT EXISTS "Users can view own documentation" ON user_documentation
    FOR SELECT USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Admins can manage all documentation" ON user_documentation
    FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'admin'));

-- Policies - readable by all
CREATE POLICY IF NOT EXISTS "Policies are viewable by all" ON policies
    FOR SELECT USING (TRUE);

-- User policies policies
CREATE POLICY IF NOT EXISTS "Users can manage own policy acknowledgments" ON user_policies
    FOR ALL USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Admins can assign policies" ON user_policies
    FOR INSERT WITH CHECK (TRUE);

-- Legal references - readable by all
CREATE POLICY IF NOT EXISTS "Legal references are viewable by all" ON legal_references
    FOR SELECT USING (TRUE);

-- User legal data policies
CREATE POLICY IF NOT EXISTS "Users can manage own legal data" ON user_legal_data
    FOR ALL USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Admins can view all legal data" ON user_legal_data
    FOR SELECT USING (EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'admin'));

-- Contracts policies
CREATE POLICY IF NOT EXISTS "Users can view own contracts" ON contracts
    FOR SELECT USING (employer_id = auth.uid() OR candidate_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Admins can manage all contracts" ON contracts
    FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'admin'));

-- Employer requests policies
CREATE POLICY IF NOT EXISTS "Users can view own requests" ON employer_requests
    FOR SELECT USING (employer_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Admins can view all requests" ON employer_requests
    FOR SELECT USING (EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'admin'));

-- Candidate recommendations policies
CREATE POLICY IF NOT EXISTS "Users can view own recommendations" ON candidate_recommendations
    FOR SELECT USING (candidate_id = auth.uid() OR recommended_by = auth.uid());

CREATE POLICY IF NOT EXISTS "Admins can manage all recommendations" ON candidate_recommendations
    FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'admin'));

-- User uploads policies
CREATE POLICY IF NOT EXISTS "Users can manage own uploads" ON user_uploads
    FOR ALL USING (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Admins can review uploads" ON user_uploads
    FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'admin'));
